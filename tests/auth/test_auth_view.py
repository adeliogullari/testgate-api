...
def test_login_with_invalid_email(client, user_factory, user):
    response = client.post(url="api/v1/auth/login", json=user_factory.stub(email="invalid_email_2024@gmail.com",
                                                                           password="password_2024").__dict__)
...
def test_login_with_invalid_password(client, user_factory, user):
    response = client.post(url="api/v1/auth/login",
                           json=user_factory.stub(email=user.email, password="invalid_password_2024").__dict__)
