...
@pytest.mark.parametrize("access_token", [Blake2bMessageDigestStrategy()], indirect=True)
def test_encode_with_valid_payload_and_key(access_token):
    token = access_token.encode(payload={'email': 'username@testgate.com'},
                                key='key',
...
def test_verify_with_exceed_expiration(access_token):
    ...
    token = access_token.encode(payload={'exp': exp,
                                         'email': 'username@testgate.com'},
                                key='key',
                                headers={'alg': 'blake2b'})
...
def test_verify_and_decode_with_valid_token_and_key(access_token):
    ...
    verified, payload, headers, signature = access_token.verify_and_decode(key='key', token=token)
...
def test_verify_and_decode_with_invalid_key(access_token):
    ...
    verified, payload, headers, signature = access_token.verify_and_decode(key='invalid_key', token=token)
