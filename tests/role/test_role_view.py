async def test_retrieve_role_by_id(client, role):
    response = client.get(f"api/v1/role/{role.id}")

    assert response.status_code == 200


async def test_search_role(client, role):
