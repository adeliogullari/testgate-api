from src.testgate.role.service import create, retrieve_by_id, retrieve_by_name, update, delete
...
def test_update(db_session, role_factory, role):
    ...
    updated_role = update(session=db_session, retrieved_role=role, role=UpdateRoleRequestModel(**role_factory.stub().__dict__))
