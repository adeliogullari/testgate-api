...
def test_retrieve_execution_by_id(client, execution):
    response = client.get(f"api/v1/execution/{execution.id}")
...
def test_update_execution(client, execution, execution_factory):
    response = client.put(
        f"api/v1/execution/{execution.id}", json=execution_factory.stub().__dict__
    )
...
def test_delete_execution(client, execution):
    response = client.delete(f"api/v1/execution/{execution.id}")
