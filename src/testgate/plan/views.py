...
role_router = APIRouter(tags=["plans"])

PlanNotFoundException = HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                      detail="A plan with this id does not exist")

PlanAlreadyExistsException = HTTPException(status_code=status.HTTP_409_CONFLICT,
                                            detail="A plan with this id already exists")


@role_router.get(path="/api/v1/plan/{id}",
                 response_model=None,
                 status_code=200,
                 dependencies=[Depends(allow_create_resource)])
def retrieve_plan_by_id(*, session: Session = Depends(get_session), id: int):
    """Retrieves plan by id."""

    retrieved_plan = retrieve_plan_by_id_service(session=session, id=id)
