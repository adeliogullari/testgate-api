...
from .schemas import RetrieveSuiteResponseModel, \
...
SuiteAlreadyExistsException = HTTPException(status_code=status.HTTP_409_CONFLICT,
...
@suite_router.post(path="/api/v1/suite",
                   ...
                   status_code=201,
                   dependencies=[Depends(allow_create_resource)])
def create_suite(*, session: Session = Depends(get_session), suite: CreateSuiteRequestModel):
    """Creates suite."""
...
@suite_router.put(path="/api/v1/suite/{id}",
                  ...
                  status_code=200,
                  dependencies=[Depends(allow_create_resource)])
def update_suite(*, session: Session = Depends(get_session), id: int, suite: UpdateSuiteRequestModel):
    """Updates suite."""
    ...
    updated_suite = update(session=session, retrieved_suite=retrieved_suite, suite=suite)
