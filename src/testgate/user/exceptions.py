...
UserEmailNotFoundException = HTTPException(
    status_code=HTTP_404_NOT_FOUND, detail="User email does not exist"
)

UserEmailAlreadyExistsException = HTTPException(
    status_code=HTTP_409_CONFLICT, detail="User email already exists"
)

InvalidPasswordException = HTTPException(
    status_code=HTTP_403_FORBIDDEN, detail="User password is invalid"
)
